...
class ContractService:
    ...
    def get_contracts_with_arrears(self, min_days: int = None) -> List[Dict]:
        """
        Obtiene todos los contratos con días de atraso >= min_days.
        ...
        logger.info(f"Consultando contratos con >= {min_days} días de atraso...")
        
        query = f"""
        SELECT 
            ca.contract_id,
            DATEDIFF(CURDATE(), ca.payment_date) AS days_overdue,
            ca.pending_balance AS total_debt,
            cs.status
        FROM 
            contract_amortization ca
        INNER JOIN 
            contract_status cs ON ca.contract_id = cs.contract_id
        WHERE 
            ca.payment_date < CURDATE()
            AND ca.pending_balance > 0
            AND cs.status = 'ACTIVO'
            AND DATEDIFF(CURDATE(), ca.payment_date) >= {min_days}
        GROUP BY 
            ca.contract_id
        ORDER BY 
            days_overdue DESC
        ...
        try:
            ...
            logger.info(f"✓ Se encontraron {len(contracts)} contratos con >= {min_days} días de atraso")
            return contracts
    ...
    def get_contracts_in_range(self, min_days: int, max_days: int) -> List[int]:
        ...
        WHERE 
            ...
            AND ca.pending_balance > 0
            AND cs.status = 'ACTIVO'
            AND DATEDIFF(CURDATE(), ca.payment_date) BETWEEN {min_days} AND {max_days}
        GROUP BY 
            ca.contract_id
        """
