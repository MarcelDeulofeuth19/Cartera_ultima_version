"""
Servicio principal de asignación de contratos a casas de cobranza.
Implementa la lógica de contratos fijos, limpieza y balanceo configurable.
"""
import logging
import math
...
class AssignmentService:
    """
    Servicio de asignación de contratos a las casas de cobranza (Cobyser/Serlefin).
    ...
    def _load_contract_blacklist(self) -> Set[int]:
        """
        Carga lista negra de contratos (TXT) cuando esta habilitada.
        """
        if not settings.BLACKLIST_ENABLED:
            logger.info("Lista negra de contratos deshabilitada por configuracion.")
            return set()

        try:
            contract_ids = blacklist_service.load_contract_ids()
            return {int(contract_id) for contract_id in contract_ids if int(contract_id) > 0}
        except Exception as error:
            logger.warning("No se pudo cargar lista negra de contratos: %s", error)
            return set()

    def _load_customer_document_blacklist(self) -> Set[str]:
        """
        Carga lista negra de clientes por cedula/documento (CSV + TXT).
        """
        blocked_documents = {
        ...
        if blocked_documents:
            logger.info(
                "Lista negra de cedula/documento cargada: %s documento(s)",
                len(blocked_documents),
            )
        return blocked_documents

    def _resolve_blocked_contract_ids_by_documents(
        self,
        blocked_documents: Set[str],
    ) -> Set[int]:
        """
        Resuelve contratos asociados a clientes en lista negra por cedula.
        """
        if not blocked_documents:
            return set()
        if self.contract_service is None:
            logger.warning(
                "No hay sesion MySQL: no se puede resolver contratos por cedula bloqueada."
            )
            return set()
        try:
            return self._require_contract_service().get_contract_ids_by_customer_documents(
                blocked_documents
            )
        except Exception as error:
            logger.error(
                "Error resolviendo contratos por cedula/documento bloqueado: %s",
                error,
            )
            raise
    ...
    def enforce_blacklist_on_active_assignments(
        ...
        try:
            ...
            for user_id, contract_id in active_rows:
                ...
                contracts_removed.setdefault(user_id_int, []).append(contract_id_int)
            ...
            logger.warning(
                "Lista negra aplicada a asignaciones activas: encontrados=%s, eliminados=%s, historial_cerrado=%s",
                stats["blocked_found_active"],
            ...
            self.postgres_session.rollback()
            logger.error(
                "Error aplicando lista negra sobre asignaciones activas: %s",
                error,
            )
            raise
    ...
    def enforce_promises_on_active_assignments(
        ...
        try:
            ...
            if not active_rows:
                logger.info(
                    "Enforcement promesas: ningun contrato con promesa activa "
                    "tiene asignacion vigente."
                )
                return stats
            ...
            logger.info(
                "Enforcement promesas activas: encontrados=%s, eliminados=%s, "
                "historial_cerrado=%s",
            ...
            self.postgres_session.rollback()
            logger.error(
                "Error aplicando enforcement de promesas activas: %s", error
            )
            raise
    ...
    # Tablas de gestión por producto (whitelist para el SQL dinámico).
    _MANAGEMENT_TABLE_BY_PRODUCT = {
        "PHONE": "managements",
        "TWIST1": "managements_twist",
        "TWIST2": "managements_twist2",
    }

    @staticmethod
    def _normalize_contract_key(raw: Any) -> Optional[Union[int, str]]:
        """
        Normaliza el identificador de contrato de una gestión.

        - None o cadena vacía -> None (se ignora).
        - entero o texto numérico -> int (Phone/Twist1).
        - texto no numérico (uuid de Twist2) -> str.
        """
        if raw is None:
            return None
    ...
    def classify_agreement_contracts(self, producto: str = "PHONE") -> Dict[str, Any]:
        """
        Clasifica contratos con COMPROMISO DE PAGO VIGENTE según la casa del asesor
        que los gestionó, usando ``alocreditindicators.users.agency_id`` como fuente
        de verdad y la gestión vigente MÁS RECIENTE por contrato.

        Reglas (app/core/management_rules.py):
          - Regla 2 -> ``result['house']`` {45: set, 81: set}: la gestión vigente es
            de una casa (agency 1=Cobyser / 2=Serlefin); el contrato debe quedar en
            ESA casa.
          - Regla 1 -> ``result['non_house']`` set: la gestión vigente es de un
            usuario que NO es casa (agency NULL, empleado interno); el contrato NO
            se asigna a las casas.

        Vigencia: acuerdo_pago con ``promise_date >= hoy`` o pago_total del mes
        calendario vigente (ver management_rules.vigente_agreement_sql).
        """
        from app.core import management_rules as mr

        result: Dict[str, Any] = {"house": {45: set(), 81: set()}, "non_house": set()}
        table = self._MANAGEMENT_TABLE_BY_PRODUCT.get(str(producto).upper())
        if not table:
            logger.warning("Producto sin tabla de gestión reconocida: %s", producto)
            return result

        cond = mr.vigente_agreement_sql("m.effect", "m.promise_date", "m.management_date")
        # No se castea contract_id: en managements/_twist es entero, pero en
        # managements_twist2 es texto (uuid de PDS, con blancos). Se normaliza en
        # Python (int cuando aplica, uuid-string para Twist2), ignorando vacíos.
        statement = text(
            f"""
            WITH ranked AS (
                SELECT
                ...
                FROM alocreditindicators.{table} m
                LEFT JOIN alocreditindicators.users u ON u.id = m.user_id
                WHERE {cond}
            )
            SELECT contract_id, agency_id
            FROM ranked
            WHERE rn = 1
            """
        )
        try:
            rows = self.postgres_session.execute(statement).mappings().all()
        except Exception as error:
            logger.error("Error clasificando gestiones (%s): %s", table, error)
            self.postgres_session.rollback()
            raise

        for row in rows:
            contract_id = self._normalize_contract_key(row["contract_id"])
            if contract_id is None:
                continue
            house_user = mr.house_user_for_agency(row["agency_id"])
            if house_user in (45, 81):
                result["house"][house_user].add(contract_id)
            else:
                result["non_house"].add(contract_id)

        logger.info(
            "Gestiones vigentes (%s): Cobyser=%s Serlefin=%s | no-casa(excluidos)=%s",
            table,
            len(result["house"][45]),
            len(result["house"][81]),
            len(result["non_house"]),
        )
        return result
    ...
    def get_current_assignments(self, producto: str = "PHONE") -> Dict[int, Set[int]]:
        ...
        try:
            ...
            logger.info(
                "Asignaciones actuales: Cobyser (45): %s, Serlefin (81): %s",
                len(current_assignments[45]),
                len(current_assignments[81]),
            )

            return current_assignments

        except Exception as e:
            logger.error("Error al consultar asignaciones actuales: %s", e)
            raise
    ...
    def _query_existing_contract_ids(
        ...
        """
        Devuelve los contract_ids ya asignados a una CASA DE COBRANZA para el producto.

        La deduplicacion se limita a los usuarios de casa (settings.USER_IDS =
        Cobyser 45 / Serlefin 81), igual que get_current_assignments. Asi, filas
        que un contrato pueda tener en contract_advisors bajo usuarios que NO son
        casa de cobranza (asignaciones heredadas de otros flujos) no bloquean su
        asignacion a la casa: sin este filtro un contrato "ya existente" bajo otro
        usuario se saltaba y nunca llegaba a la casa de cobranza.
        """
    ...
    def assign_franja_by_parity(
        self,
        excluded_from_assignment: Optional[Set[int]] = None,
        producto: str = "PHONE",
    ) -> Dict[str, Any]:
        """
        Asigna la franja 31-60 (buckets 31_45 y 46_60) por PARIDAD de cédula:
        impar -> Cobyser (45), par -> Serlefin (81).

        Pasada ADITIVA e independiente del balanceo 60/40 de 61-240 (que solo
        consulta >=61 días, sin solape). save_assignments deduplica contra
        contract_advisors por casa/producto (no mueve lo ya asignado). El historial
        se etiqueta 'CEDULAS_IMPAR' (Cobyser) o 'CEDULAS_PAR' (Serlefin).
        """
        stats: Dict[str, Any] = {
        ...
        if not settings.FRANJA_COBYSER_ENABLED:
            logger.info("Franja deshabilitada por configuracion (FRANJA_COBYSER_ENABLED=False).")
            return stats

        cobyser_user_id = int(settings.FRANJA_COBYSER_USER_ID)
        serlefin_user_id = int(settings.SERLEFIN_PRIMARY_USER_ID)
        excluded = {
            int(contract_id)
        ...
        franja_contracts = contract_service.get_franja_contracts_by_parity(
        ...
        if not franja_contracts:
            logger.info("Franja (%s): no hay contratos 31-60 con cedula para asignar.", producto)
            return stats

        franja_assignments: Dict[int, List[int]] = {cobyser_user_id: [], serlefin_user_id: []}
        franja_days_map: Dict[int, int] = {}
        tipo_map: Dict[int, str] = {}
        for contract in franja_contracts:
            contract_id = int(contract["contract_id"])
            franja_days_map[contract_id] = int(contract["days_overdue"])
            if contract.get("parity") == "impar":
                franja_assignments[cobyser_user_id].append(contract_id)
                tipo_map[contract_id] = "CEDULAS_IMPAR"
            else:
                franja_assignments[serlefin_user_id].append(contract_id)
                tipo_map[contract_id] = "CEDULAS_PAR"
        ...
        stats["assigned_cobyser"] = int(insert_stats.get("inserted_cobyser", 0))
        stats["assigned_serlefin"] = int(insert_stats.get("inserted_serlefin", 0))
        stats["insert_stats"] = insert_stats
        logger.info(
            "Franja por paridad (%s): candidatos=%s, insertados=%s (Cobyser impar=%s / Serlefin par=%s)",
            producto,
            stats["candidates"],
            stats["assigned"],
            stats["assigned_cobyser"],
            stats["assigned_serlefin"],
        )
        return stats
    ...
    def execute_assignment_process(self) -> Dict:
        ...
        try:
            ...
            blocked_contract_ids: Set[int] = self._load_contract_blacklist()
            blocked_documents: Set[str] = self._load_customer_document_blacklist()
            blocked_contract_ids.update(
                self._resolve_blocked_contract_ids_by_documents(blocked_documents)
            )
            ...
            logger.info(
                "Contratos con promesa activa excluidos de asignacion: %s",
            ...
            new_assignments, contracts_days_map = self.balance_assignments(
                ...
                blocked_contract_ids=excluded_from_assignment,
            )
            ...
            # Franja 31-60 por paridad de cédula: impar -> Cobyser (45),
            # par -> Serlefin (81). Pasada aditiva, independiente del 60/40 de 61-240.
            results["franja_cobyser_stats"] = self.assign_franja_by_parity(
    ...
    def send_completion_notification(self, results: Dict[str, Any]) -> bool:
        ...
        <body style="font-family:Arial,sans-serif">
          ...
          <p><strong>Promesas activas detectadas (excluidas de asignacion):</strong> Cobyser {fixed_counts.get("cobyser_45", 0)} / Serlefin {fixed_counts.get("serlefin_81", 0)} / Total excluidos: {results.get("promise_excluded_count", 0)}</p>
    ...
    def finalize_all_active_assignments(self) -> Dict[str, int]:
        """
        Cierre masivo: cierra todas las asignaciones activas del dia.
        1) Consulta datos actuales de MySQL (dias atraso, estado) para enriquecer el cierre.
        2) Marca fecha_terminal en historial activo con metadatos terminales.
        3) Elimina filas activas en contract_advisors.

        Optimizado para bajo consumo de RAM: libera estructuras intermedias
        tan pronto como dejan de necesitarse.
        """
        stats = {
            ...
            "enriched_from_mysql": 0,
        }

        try:
            # 1. Obtener asignaciones activas (solo IDs, bajo consumo de RAM)
            active_rows = self.postgres_session.query(
                ContractAdvisor.user_id,
                ContractAdvisor.contract_id,
            ).all()
            ...
            if not active_rows:
                logger.info("Cierre masivo: no hay asignaciones activas para cerrar.")
                return stats

            stats["active_assignments_found"] = len(active_rows)

            # 2. Agrupar por user_id y recopilar contract_ids unicos
            contracts_removed: Dict[int, List[int]] = {}
            unique_contract_ids_set: Set[int] = set()
            for user_id, contract_id in active_rows:
                user_id_int = int(user_id)
                contract_id_int = int(contract_id)
                contracts_removed.setdefault(user_id_int, []).append(contract_id_int)
                unique_contract_ids_set.add(contract_id_int)
            del active_rows

            # 3. Enriquecer cierre con datos actuales de MySQL
            terminal_metadata: Optional[Dict[int, Dict[str, Any]]] = None

            if self.contract_service is not None:
                unique_ids = list(unique_contract_ids_set)
                logger.info(
                    "Cierre masivo: enriqueciendo %s contratos con datos de MySQL...",
                    len(unique_ids),
                )
                ...
                state_map = self.contract_service.get_current_state_for_contracts(
                    unique_ids
                )

                terminal_metadata = {}
                for cid in unique_ids:
                    days = days_map.get(cid, 0)
                    terminal_metadata[cid] = {
                        "dias_atraso_terminal": days,
                        "dpd_terminal": get_dpd_range(days),
                        "estado_actual": state_map.get(cid, "SIN_ESTADO"),
                        "tipo": "CIERRE_MASIVO",
                    }

                stats["enriched_from_mysql"] = len(terminal_metadata)
                del days_map, state_map, unique_ids
                logger.info(
                    "Cierre masivo: enriquecimiento MySQL completado (%s contratos).",
                    stats["enriched_from_mysql"],
                )
            else:
                logger.warning(
                    "Cierre masivo: sin sesion MySQL, cerrando sin datos de atraso/estado."
                )
            del unique_contract_ids_set

            # 4. Cerrar historial con metadatos enriquecidos
            history_stats = self.history_service.close_assignments(
                contracts_removed,
                terminal_metadata=terminal_metadata,
            )
            stats["history_closed"] = int(history_stats.get("total_closed", 0))
            stats["history_updated"] = int(history_stats.get("updated", 0))
            stats["history_inserted"] = int(history_stats.get("inserted", 0))
            del terminal_metadata, contracts_removed

            # 5. Eliminar de contract_advisors
            deleted_count = self.postgres_session.query(ContractAdvisor).delete(
                synchronize_session=False
            )
            self.postgres_session.commit()
            stats["deleted_from_contract_advisors"] = int(deleted_count or 0)

            logger.info(
                "Cierre masivo completado: activos=%s, historial_cerrado=%s, "
                "enriquecidos=%s, eliminados=%s",
            ...
            self.postgres_session.rollback()
            logger.error("Error en cierre masivo de asignaciones: %s", error)
            raise
